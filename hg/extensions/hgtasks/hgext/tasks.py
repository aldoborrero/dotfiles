...
def task(ui, repo, task=None, rev=None, force=False, delete=False, info=None,
    ...
    Use 'hg update [TASK]' to update to the tip node of a task and set that
    ...
    if task == None:
        ...
        if len(tasks) == 0:
            ...
            for t, tinfo in tasks.iteritems():
                ...
                # don't display completed tasks unless told to
                if tinfo['state'] == 2 and not all: continue

                tnode = tasknode(repo, tinfo)

                # short format with --quiet
                if ui.quiet:
                    ui.write('%s\n' % t)
                    continue
...
def uisetup(ui):
    ...
    entry = extensions.wrapcommand(commands.table, 'outgoing', tasksoutgoing)
    entry[1].append(('', 'completed-tasks', None,
        _('changesets for completed tasks only')))
    entry[1].append(('', 'all-tasks', None,
        _('all changesets including those from incomplete tasks')))
...
def outgoingnodes(ui, repo, dest=None, **opts):
    # get remote repo
    dest, branches = hg.parseurl(ui.expandpath(dest or 'default-push',
        dest or 'default'), opts.get('rev'))
    try:
        remote = hg.repository(hg.remoteui(repo, opts), dest)
    except AttributeError:
        cmdutil.setremoteconfig(ui, opts)
        remote = hg.repository(ui, dest)

    revs, checkout = hg.addbranchrevs(repo, remote, branches, None)

    # suppress output
    ui.pushbuffer()
    out = discovery.findoutgoing(repo, remote, None)
    ui.flush()
    ui.popbuffer()
